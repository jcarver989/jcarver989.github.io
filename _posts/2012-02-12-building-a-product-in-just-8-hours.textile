...
We decided to build a news recommender system. The system would use "Bizo's rich business data":http://www.bizo.com/bizographics/index to suggest personalized content for visitors based on things like their industry, company size and seniority, ex: people in a marketing industry would see news relevant to marketers, while people in software would see tech related news items. The goal for the end of the day was to have a small webapp that would show the top 10 news articles for visitors' "business segments":http://www.bizo.com/marketer/targetable_segments

After a bit more discussion, we decided to split tasks up into five groups of two engineers:

* **News Discovery Team**   - discover the top news urls grouped by using data gathered from our network
* **URL Scraping Team**     - given a url representing a news item scrape the page contents and store them for later use 
* **Data Extraction Team**  - extract the news item's title &amp; description using the previously scraped pages  
* **Backend Team**   - backend architecture for the webapp that fetches serves the news content generated in the previous steps 
...
* **News Discovery Team** Worked out a simple ranking algorithm and started writing the "Hive":https://cwiki.apache.org/confluence/display/Hive/Home%3bjsessionid=E3548DACBC8C6E9EC8EBE15F65B8CF57 script to extract news items 

* **URL Scraping Team** Started out in Scala hacking up a script to scrape & download url content.

* **Data Extraction Team** Decided to try out the "Pismo":http://rubygems.org/gems/pismo gem to extract summaries and titles from the scraped html content.
...
Using some sample data generated by the first three teams we were able to get a rough ui working pretty quickly. Our side of things turned out to be pretty straightforward and involved three AJAX requests. One was to retrieve a list of news items grouped by segment from the Scalatra web server, one to get a list of the current targetable segments from the "Bizo API":http://developer.bizo.com/ and another call to the API to retrieve a visitor's business segments (bizographics). 
